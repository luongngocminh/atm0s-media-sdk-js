...
import type { IRPC, RpcEvents, RpcMessage, RpcRequests, RpcResponse } from '../interfaces/rpc';
import { RealtimeSocketState } from '../interfaces/rtsocket';
...
export class RPC implements IRPC {
  ...
  constructor(private _socket: RealtimeSocket) {
  ...
  private _process = (msg: string) => {
    ...
    switch (type) {
      case 'event':
        this._handleEvent(json.cmd!, json.data);
        break;
      case 'answer':
        this._handleAnswer(json.seq, json.success!, json.data);
        break;
      case 'request':
        this._socket.send(
          JSON.stringify({
